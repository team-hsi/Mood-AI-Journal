...
export default function Editor({ entry }: EditorProps) {
  const { content, setContent, analysis, analysisLoading, handleNewAnalysis } = useEditor(entry)

  return (
    <ResizablePanelGroup direction="horizontal" className="h-full w-full">
      <ResizablePanel defaultSize={75} className="px-3 py-1 max-sm:flex max-sm:flex-col max-sm:gap-4 max-sm:pl-2 max-sm:pr-8">
        <ContentEditor
          content={content}
          onChange={setContent}
          isMobile={false}
